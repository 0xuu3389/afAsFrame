import time

import requests

class scriptReq:
    def __init__(self):
        self.shellUrl = 'http://127.0.0.1:8766/do?type=Shell'
        pass

    def shell_user_web(self,url,data):
        try:
            # 设置请求头
            headers = {
                'Content-Type': 'application/json'
            }
            # 发送POST请求
            response = requests.post(url=url, headers=headers, json=data)
            # 打印响应内容
            # 打印响应内容
            presjson = response.json()
            return  presjson
        except requests.exceptions.RequestException as e:
            # 捕获并处理网络请求中的异常
            print('网络异常:', e)

    def shell_user_kill(self,packageName):
        try:
            # 设置请求头
            headers = {
                'Content-Type': 'application/json'
            }
            # 构建要发送的JSON payload
            data = {
                'shell': f'am force-stop {packageName}',
                'is_root': False
            }
            # 发送POST请求
            response = requests.post(url=self.shellUrl, headers=headers, json=data)
            # 打印响应内容
            print(response.text)
            return True
        except requests.exceptions.RequestException as e:
            # 捕获并处理网络请求中的异常
            print('网络异常:', e)
            return False

    def shell_user_get(self,url):
        try:
            # 设置请求头
            headers = {
                'Content-Type': 'application/json'
            }
            # 发送POST请求
            response = requests.get(url=url, headers=headers)
            # 打印响应内容
            presjson = response.json()
            return  presjson
        except requests.exceptions.RequestException as e:
            # 捕获并处理网络请求中的异常
            print('网络异常:', e)

    def shell_user_post(self,url,data):
        try:
            # 设置请求头
            headers = {
                'Content-Type': 'application/json'
            }
            # 发送POST请求
            response = requests.post(url=url, headers=headers, json=data)
            # 打印响应内容
            presjson = response.json()
            return presjson
        except requests.exceptions.RequestException as e:
            # 捕获并处理网络请求中的异常
            print('网络异常:', e)

    def check_task_req(self, url):
        while True:
            try:
                response = requests.get(url, timeout=10)
                response.raise_for_status()  # 检查请求是否成功
                resjson = response.json()  # 解析 JSON 数据
                print('任务检测结果==>', resjson)
                return resjson
            except requests.exceptions.RequestException as e:
                print(f'请求异常: {e}')
                print('等待 5 秒后重试...')
                time.sleep(5)  # 等待 5 秒后重试

    def task_Post_Req(self, url, headers, data):
        while True:
            try:
                # 发送 POST 请求
                response = requests.post(url, headers=headers, json=data, timeout=10)
                response.raise_for_status()  # 检查请求是否成功
                resjson = response.json()  # 解析 JSON 数据
                print('任务检测结果==>', resjson)
                return resjson  # 请求成功后返回结果
            except requests.exceptions.RequestException as e:
                print(f'请求异常: {e}')
                print('等待 5 秒后重试...')
                time.sleep(5)  # 等待 5 秒后重新尝试